    ...
    meals.then(data=>{
        if (data.meals) {
            data.meals.forEach((element) => {
                ...
                if (isFav) {
                    html += `
                <div id="card" class="card mb-3" style="width: 15rem; height: 20rem">
                    <img src="${element.strMealThumb}" class="card-img-top" style="height:11rem alt="...">
                    <div class="card-body">
          ...
          <div id="meal-details" class="mb-5">
            <div id="meal-header" class="d-flex justify-content-around flex-wrap  ">
              <div id="meal-thumbail"  >
                <img class="mb-2" src="${data.meals[0].strMealThumb}" alt="" style="height: 11rem" srcset="">
              </div>
              <div id="details">
    ...
    if (arr.length==0) {
        ...
        for (let index = 0; index < arr.length; index++) {
            await fetchMealsFromApi(url,arr[index]).then(data=>{
                html += `
                <div id="card" class="card mb-3" style="width: 15rem height:20rem;">
                    <img src="${data.meals[0].strMealThumb}" class="card-img-top" height:11rem alt="...">
                    <div class="card-body">
